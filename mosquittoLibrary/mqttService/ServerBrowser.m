//
//  ServerBrowser.m
//  Bonjour search for MQTT Broker
//


//Container view will start the service discovery allready at starting up, if we get a connection to a
//Broker service will be stopped, if not, and if we find one, that will be used, and last good know host
   ...
   // struct sockaddr_in addressClient;
    ...
    switch( addressGeneric->sa_family ) {
        case AF_INET: {
            ...
            BrokerIp=[NSString stringWithFormat:@"%s",inet_ntop(AF_INET, &ip4->sin_addr, dest, sizeof dest)];
#ifdef trace_on
            NSLog(@"SERVER BROWSER: Broker IP %@",BrokerIp);
#endif
        }
            break;
        ...
        case AF_INET6: {
            ...
            //BrokerIp=[NSString stringWithFormat:@"%s",inet_ntop(AF_INET6, &ip6->sin6_addr, dest, sizeof dest)];
        }
            break;
        default:
